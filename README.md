...
| ✅ **Pruebas** | **729** en la suite normal · **+50 con navegador** (42 de pantallas reales y 8 de tarjetas vivas), que corren **sin robot** contra el doble · y **4 que sí lo necesitan**: barrido real, dos de acciones y la parada en marcha |
...
| 🆕 **EL TERMINAL, construido** | 2026-08-14. Era el producto y la mitad bloqueada de la aplicación. Ahora el alumno abre una práctica **del robot**, la edita y la ejecuta, contesta a sus `input()` y la para — sin SSH. Tres piezas: el **testigo Ed25519** que firma Next y verifica el robot (cruzado de verdad, en dos lenguajes), el **agente de sesión** en el 9443 con PTY y señales, y la pantalla. 🔴 **Y nada de esto ha tocado un robot**: lo verificado es el núcleo (31 pruebas), el protocolo y la pantalla contra un doble; el PTY y las distancias con cinta están en [`VALIDAR_CON_EL_ROBOT.md`](VALIDAR_CON_EL_ROBOT.md) §4, con qué refutaría cada una |
| 🔴 **Y lo que el terminal ABRE, escrito en la pantalla** | El programa del alumno corre con `rclpy` **nativo**: desde ahí alcanza `raw_motors`, `move_timed` y `set_ir_mode('following')` — los caminos que **se saltan el `collision_monitor`** y que la lista blanca cierra al navegador. La frase «`raw_motors` ya no es alcanzable, 0,00 cm verificado» **deja de ser cierta mientras haya un programa corriendo**. No se puede impedir sin quitarle Python al alumno: lo que se hace es decirlo |
| 🔴 **La cadena de tres eslabones, cerrada** | La pantalla la pintaba entera y en orden desde el 2026-08-04 — F0 (medir el aula) → agente de sesión → terminal — porque decirlo era más útil que un hueco. El primero se descartó como riesgo el 2026-08-10, el segundo se escribió el 2026-08-14, y el tercero es esto. 📝 Lo que queda **ya no es un eslabón, es una medida** |
